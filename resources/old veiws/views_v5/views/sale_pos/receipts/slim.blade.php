...
<div class="ticket">
    ...
    <table style="margin-top: 25px !important" class="border-bottom width-100 table-f-12 mb-10">
        ...
        <tr>
            ...
            @if(empty($receipt_details->hide_price))
                ...
                @endif
                {{-- Tax --}}
                <th class="tax_value text-right">قيمة الضريبة</th>
        ...
        @forelse($receipt_details->lines as $line)
            <tr>
                ...
                @if(empty($receipt_details->hide_price))
                    ...
                    @endif
                    {{-- Tax --}}
                    <td class="tax_value text-right">{{$line['tax']}}</td>
    ...
    @php
        $subtotal = (float)preg_replace("/[^0-9\.]/", "", $receipt_details->subtotal_exc_tax);
        $taxes = (float)preg_replace("/[^0-9\.]/", "", array_values($receipt_details->taxes)[1]);
        $method = $receipt_details->payments[0]['method'];
    @endphp
    ...
    @if(empty($receipt_details->hide_price))
        ...
        <div class="flex-box">
            ...
            <p class="width-50 text-right sub-headings">
                {{array_values($receipt_details->taxes)[1]}}
            </p>
        </div>
        ...
        <div class="flex-box">
            ...
            <p class="width-50 text-right sub-headings">
                {{$receipt_details->currency_symbol}} {{ $subtotal + $taxes }}
            </p>
        </div>
