...
{   
    ...
    public function modifyAdminMenu()
    {
        ...
        $is_spreadsheet_enabled = (boolean)$module_util->hasThePermissionInSubscription($business_id, 'spreadsheet_module');
        ...
        if ($is_spreadsheet_enabled && auth()->user()->can('access.spreadsheet')) {
            Menu::modify('admin-sidebar-menu', function ($menu) use ($background_color) {
                $menu->url(
                    action('\Modules\Spreadsheet\Http\Controllers\SpreadsheetController@index'),
        ...
        if ($notification->type == 'Modules\Spreadsheet\Notifications\SpreadsheetShared') {
            ...
            if (!empty($spreadsheet)) {
                ...
                $notification_datas = [
                    ...
                    'link' => action('\Modules\Spreadsheet\Http\Controllers\SpreadsheetController@show', [$spreadsheet->id]),
