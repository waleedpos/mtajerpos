...
class HomeController extends Controller
{
    ...
    public function submitRegisteration (Request $request)
    {
        $data = request()->all();
        
        $rules = [
            'fullName.first' => 'required|string',
            'fullName.last' => 'required|string',
            'phoneNumber.area' => 'required|string',
            'phoneNumber.phone' => 'required|string',
...
if ($file) {
    $path = Storage::disk('public_web')->putFile('orders', $file);
    $imageUrl = Storage::disk('public_web')->url($path);
    // Do something with the $url, such as saving it in the database
}
        
        $html = (new MailMessage)
            ->greeting('تم تسجيل طلب جديد')
            ->line('<br> <br> البيانات: <br>')
            ->line('الاسم: ' . $request->input('fullName.first') . ' ' . $request->input('fullName.last'))
            ->line('رقم جوال المنشأه: ' . $request->input('phoneNumber.area') . '-' . $request->input('phoneNumber.phone'))
            ->line('رقم السجل التجاري: ' . $request->input('org_number'))
            ->line('البريد : ' . $request->input('email'))
            ->line('العنوان: ' . $request->input('address'))
            ->line('ملاحظات: ' . $request->input('notes'))
            ->line('السعر المتفق عليه: ' . $request->input('price'))
            ->line('الموعد : ' . $request->input('date'))
            ->line('<img width="100" height="100" src="' . $imageUrl . '">');
        
        Mail::send('emails.orderRegisteration', ['html' => $html], function ($message) use ($request, $imageUrl) {
            $message->to('support@mtrtech.net')
                ->subject(' طلب تسجيل جديد من ' . $request->input('fullName.first') . '  -  ' . $request->input('fullName.last'));
        });
        
        return '<div class="success-message" style="padding: 10px; background-color: #4CAF50; color: white; font-weight: bold; text-align: center;"> تم تسجيل طلبك سنقوم بالإتصال بك </div>';
